...
<template>
    <article :style="fontVariables" class="qr-template qr-template-pink-landscape relative mx-auto aspect-[1.4142/1] h-full max-h-full w-auto max-w-full overflow-hidden rounded-[2rem] shadow-[0_34px_80px_rgba(76,41,52,0.15)]">
        <div class="qr-template__art absolute inset-0" />
        <div class="qr-template__wash absolute inset-0" />

        <div class="relative z-10 grid h-full grid-cols-[minmax(0,1fr)_minmax(10rem,31cqw)] gap-[3.6cqw] px-[6.5%] py-[7%] text-left text-[#38232d]">
            <div class="flex h-full flex-col justify-between">
                ...
                <div class="space-y-[3.2cqh]">
                    <p class="qr-template__message">
                        {{ message }}
                    </p>

                    <footer class="qr-template__footer w-full max-w-[58cqw] border-t border-[#7a5260]/16 pt-[2cqh]">
...
.qr-template__subtitle {
    ...
    font-size: clamp(0.46rem, calc(var(--qr-unit) * 1.2), 0.82rem);
    font-weight: 800;
    letter-spacing: clamp(0.12em, calc(var(--qr-unit) * 0.08), 0.24em);
...
.qr-template__title {
    ...
    font-size: clamp(1.6rem, calc(var(--qr-unit) * 6.5), 4.4rem);
    font-weight: 600;
    line-height: 0.84;
...
.qr-template__slogan {
    margin-top: 1cqh;
    font-family: var(--qr-body-font);
    font-size: clamp(0.68rem, calc(var(--qr-unit) * 1.9), 1.02rem);
    line-height: 1.45;
...
.qr-template__qr-frame {
    ...
    padding: clamp(0.3rem, calc(var(--qr-unit) * 0.58), 0.58rem);
...
.qr-template__message {
    ...
    font-family: var(--qr-body-font);
    font-size: clamp(0.66rem, calc(var(--qr-unit) * 1.55), 0.92rem);
    line-height: 1.58;
...
.qr-template__event-title {
    font-size: clamp(0.8rem, calc(var(--qr-unit) * 1.85), 1.18rem);
    font-weight: 600;
