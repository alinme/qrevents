...
<template>
    <article
        :style="fontVariables"
        class="qr-template qr-template-pink-landscape relative mx-auto aspect-[1748/1240] h-full max-h-full w-auto max-w-full overflow-hidden rounded-[2rem] shadow-[0_34px_80px_rgba(76,41,52,0.15)]"
    >
        <div class="qr-template__art absolute inset-0" />

        <header
            class="absolute left-[6%] w-[54%] text-left text-[#38232d]"
        ...
        <p
            class="qr-template__message absolute left-[6%] w-[50%]"
        ...
        <footer
            class="qr-template__footer absolute left-[6%] w-[46%] border-t border-[#7a5260]/16 pt-[1.6cqh]"
...
.qr-template__title {
    ...
    font-size: clamp(1.8rem, calc(var(--qr-unit) * 7.6), 4.8rem);
    font-weight: 600;
    line-height: 0.84;
...
.qr-template__message {
    ...
    font-family: var(--qr-body-font);
    font-size: clamp(0.72rem, calc(var(--qr-unit) * 2), 1.05rem);
    line-height: 1.65;
