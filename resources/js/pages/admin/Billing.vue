...
import { CreditCard, ShieldAlert } from 'lucide-vue-next';
...
const props = defineProps<{
    ...
    attentionEvents: BillingAttentionRow[];
...
const search = ref('');
const queueFilter = ref<'all' | 'locked' | 'expired' | 'overdue' | 'awaiting'>('all');
const cleanupDialogOpen = ref(false);
const selectedCleanupEvent = ref<BillingAttentionRow | null>(null);
...
const filteredAttentionEvents = computed(() => {
    const term = search.value.trim().toLowerCase();

    return props.attentionEvents.filter((event) => {
        ...
        const matchesFilter = queueFilter.value === 'all'
            || (queueFilter.value === 'locked' && event.queueLabel === 'Locked')
            || (queueFilter.value === 'expired' && event.queueLabel === 'Expired')
            || (queueFilter.value === 'overdue' && event.queueLabel === 'Overdue')
            || (queueFilter.value === 'awaiting' && event.queueLabel === 'Awaiting payment');
...
const openCleanupDialog = (event: BillingAttentionRow, action: 'clear_export' | 'purge_media'): void => {
...
const updateCleanupReview = (
    event: BillingAttentionRow,
    reviewState: 'approved' | 'protected' | 'clear',
): void => {
            ...
            <div class="mx-auto flex max-w-7xl flex-col gap-6 p-4 md:p-6">
                <section class="overflow-hidden rounded-[2rem] border border-black/5 bg-white shadow-sm">
                    <div class="border-b border-black/5 bg-[linear-gradient(135deg,#171411_0%,#2d251f_46%,#5f533f_100%)] px-6 py-8 text-white md:px-8">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                            <div class="space-y-2">
                                ...
                                <h1 class="text-3xl font-semibold tracking-tight md:text-4xl">Billing</h1>
                                <p class="max-w-2xl text-sm leading-6 text-white/72 md:text-base">
                                    Manual billing control for the MVP: see who is unpaid, who is locked, and which event was paid most recently.
                                </p>
                            </div>
                    ...
                    <div class="grid gap-4 p-5 md:grid-cols-5 xl:p-6">
                        ...
                        <div class="rounded-2xl border border-black/6 bg-[#fbfaf7] p-4">
                            <p class="text-sm font-medium text-zinc-500">Recent payments</p>
                            <p class="mt-2 text-3xl font-semibold text-[#171411]">{{ recentPayments.length }}</p>
                        </div>
                        <div class="rounded-2xl border border-black/6 bg-[#fbfaf7] p-4">
                ...
                <div class="grid gap-6 xl:grid-cols-[1.08fr_0.92fr]">
                    <section class="rounded-[2rem] border border-black/5 bg-white p-5 shadow-sm md:p-6">
                        <div class="flex items-center gap-3">
                            <div class="rounded-2xl bg-[#f8ecdc] p-3 text-[#7b5a2c]">
                                <ShieldAlert class="size-5" />
                            </div>
                            <div>
                                <p class="text-sm font-semibold uppercase tracking-[0.2em] text-zinc-500">Needs attention</p>
                                <h2 class="text-2xl font-semibold tracking-tight text-[#171411]">Unpaid and locked events</h2>
                            </div>
                        </div>
                        ...
                        <div class="mt-5 flex flex-col gap-3 md:flex-row">
                            <input
                                v-model="search"
                                type="search"
                                placeholder="Search by event, owner, email, note, or plan"
                                class="h-11 flex-1 rounded-full border border-black/10 bg-[#fbfaf7] px-4 text-sm text-[#171411] outline-none transition focus:border-[#c79a5b] focus:ring-2 focus:ring-[#e8c892]"
                            >
                            <select
                                v-model="queueFilter"
                        ...
                        <div v-if="filteredAttentionEvents.length === 0" class="py-12 text-center">
                            <div class="mx-auto flex max-w-md flex-col items-center gap-4">
                                <div class="rounded-full bg-[#fbfaf7] p-4 text-[#171411]">
                                    <CreditCard class="size-7" />
                                </div>
                                <div class="space-y-2">
                                    <h2 class="text-xl font-semibold text-[#171411]">No matching billing items</h2>
                                    <p class="text-sm leading-6 text-zinc-600">
                                        Adjust the search or queue filter to widen the results.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-else class="mt-5 grid gap-4">
                            <article v-for="event in filteredAttentionEvents" :key="event.id" class="rounded-[1.5rem] border border-black/6 bg-[#fcfbf8] p-5">
                                <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                                    <div class="space-y-1">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <h3 class="text-xl font-semibold text-[#171411]">{{ event.name }}</h3>
                                    ...
                                    <div class="rounded-2xl border border-black/6 bg-white px-4 py-3 text-sm text-zinc-600">
                                        <p><span class="font-medium text-[#171411]">Payment due:</span> {{ formatDateTime(event.paymentDueAt) }}</p>
                                        <p class="mt-1"><span class="font-medium text-[#171411]">Paid at:</span> {{ formatDateTime(event.paidAt) }}</p>
                                        <p class="mt-1"><span class="font-medium text-[#171411]">Storage:</span> {{ formatBytes(event.storage.usedBytes) }} / {{ formatBytes(event.storage.limitBytes) }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="mt-4 rounded-2xl border border-dashed border-[#decfb6] bg-[#fbf6ee] px-4 py-3 text-sm text-zinc-700">
                                    ...
                                    <p v-if="event.cleanup.candidateAt" class="mt-1 text-zinc-500">
                                        Candidate after {{ formatDateTime(event.cleanup.candidateAt) }}
                                    </p>
                                </div>
                                ...
                                <div class="mt-4 flex flex-wrap gap-2">
                                    ...
                                    <Link :href="event.links.settings" class="rounded-full border border-black/10 bg-[#171411] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#2b241d]">
                                        Open billing settings
                                    </Link>
                                    <Link :href="event.links.event" class="rounded-full border border-black/10 bg-white px-4 py-2 text-sm font-medium text-[#171411] transition hover:bg-[#f7f1e7]">
                                        Open event
                                    </Link>
                                </div>
                            </article>
                    ...
                    <section class="rounded-[2rem] border border-black/5 bg-white p-5 shadow-sm md:p-6">
                        <div>
                            <p class="text-sm font-semibold uppercase tracking-[0.2em] text-zinc-500">Recently paid</p>
                            <h2 class="mt-1 text-2xl font-semibold tracking-tight text-[#171411]">Latest confirmed payments</h2>
                        </div>

                        <div v-if="recentPayments.length === 0" class="py-12 text-center">
                            <div class="mx-auto flex max-w-md flex-col items-center gap-4">
                                <div class="rounded-full bg-[#fbfaf7] p-4 text-[#171411]">
                                    <CreditCard class="size-7" />
                                </div>
                                <div class="space-y-2">
                                    <h2 class="text-xl font-semibold text-[#171411]">No payments recorded yet</h2>
                                    <p class="text-sm leading-6 text-zinc-600">
                                        Marking an event as paid in event settings will surface it here.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-else class="mt-5 grid gap-3">
                            <article v-for="event in recentPayments" :key="event.id" class="rounded-[1.35rem] border border-black/6 bg-[#fcfbf8] p-4">
                                <h3 class="text-lg font-semibold text-[#171411]">{{ event.name }}</h3>
                                <p class="mt-1 text-sm text-zinc-600">{{ event.ownerEmail }}</p>
                                <p class="mt-2 text-sm text-zinc-500">{{ event.planName }} · {{ event.planPriceLabel }}</p>
                                <div class="mt-3 space-y-1 text-sm text-zinc-600">
                                    <p><span class="font-medium text-[#171411]">Paid:</span> {{ formatDateTime(event.paidAt) }}</p>
                                    <p><span class="font-medium text-[#171411]">Due:</span> {{ formatDateTime(event.paymentDueAt) }}</p>
                                    <p><span class="font-medium text-[#171411]">Storage:</span> {{ formatBytes(event.storage.usedBytes) }}</p>
                                </div>
                                <div class="mt-4">
                                    <Link :href="event.links.settings" class="text-sm font-semibold text-[#171411] hover:text-[#7b5a2c]">
                                        Open event billing
                                    </Link>
                                </div>
                            </article>
        ...
        <DialogContent class="sm:max-w-lg">
            ...
            <div v-if="selectedCleanupEvent" class="space-y-4">
                <div class="rounded-2xl border border-black/6 bg-[#fbfaf7] p-4 text-sm text-zinc-700">
                    ...
                    <p class="mt-1"><span class="font-medium text-[#171411]">Queue:</span> {{ selectedCleanupEvent.queueLabel }}</p>
