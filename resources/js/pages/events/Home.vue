...
const formatDateTime = (
    value: string | null,
    fallback = t('event_home.fallback.unknown'),
): string => {
    if (!value) {
        return fallback;
    }
...
const uploadWindowLabel = computed(() => {
    if (
        !props.currentEvent.uploadWindowStartsAt ||
        !props.currentEvent.uploadWindowEndsAt
    ) {
...
const billingActionLabel = computed(() =>
    props.currentEvent.billing.canManage
        ? t('event_home.actions.open_billing')
        : t('event_home.actions.view_billing'),
);
...
const mediaExportHint = computed(() => {
    ...
    if (mediaExportReady.value) {
        return props.currentEvent.mediaExport.completedAt
            ? t('event_home.export.hint_ready_since', {
                  date: formatDateTime(
                      props.currentEvent.mediaExport.completedAt,
                  ),
              })
    ...
    if (props.currentEvent.mediaExport.status === 'failed') {
        return (
            props.currentEvent.mediaExport.error ||
            t('event_home.export.hint_failed')
        );
    }
...
const summaryItems = computed(() => [
    {
        ...
        detail: props.dashboardStats.lastUploadAt
            ? t('event_home.summary.last_upload', {
                  date: formatDateTime(props.dashboardStats.lastUploadAt),
              })
    ...
    },
    {
        ...
        detail: t('event_home.summary.remaining', {
            count: props.dashboardStats.uploadRemaining,
        }),
        icon: ImageIcon,
    },
    {
        ...
        detail: t('event_home.summary.approved', {
            count: props.dashboardStats.approvedCount,
        }),
        icon: Clock3,
    },
    {
        ...
        detail: t('event_home.summary.free', {
            amount: formatBytes(props.dashboardStats.storageRemainingBytes),
        }),
        icon: CheckCircle2,
    },
]);
...
const recentUploadSummary = (upload: RecentUpload): string => {
    ...
    return (
        upload.message?.trim() ||
        t(`event_home.recent.kind_upload.${upload.kind}`)
    );
};

const moderationToneClass = (
    status: RecentUpload['moderationStatus'],
): string => {
    switch (status) {
        case 'approved':
        ...
        if (nextLatestUploadId > latestKnownUploadId.value) {
            ...
            toast.success(
                newUploadCount === 1
                    ...
                    : t('event_home.activity.new_upload_plural', {
                          count: newUploadCount,
                      }),
            );
        }

        latestKnownUploadId.value = Math.max(
            latestKnownUploadId.value,
            nextLatestUploadId,
        );
    },
);
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="min-h-full bg-brand-canvas">
            <div
                class="mx-auto flex w-full max-w-6xl flex-col gap-5 p-4 md:p-6"
            >
                <section
                ...
                >
                    ...
                    >
                        <div class="flex items-start gap-3">
                            <div
                                class="rounded-full bg-white p-2 text-amber-700 shadow-sm"
                            >
                ...
                >
                    ...
                    >
                        <div class="max-w-3xl">
                            <p
                                class="text-[0.68rem] font-semibold tracking-[0.22em] text-zinc-500 uppercase"
                            >
                                {{ t('event_home.hero.kicker') }}
                            </p>
                            <h1
                                class="mt-2 text-xl font-semibold tracking-tight text-brand-ink sm:text-2xl"
                            >
                                {{ currentEvent.name }}
                            </h1>
                            ...
                            <div class="mt-3 flex flex-wrap items-center gap-2">
                                <span
                                    class="text-xs font-semibold tracking-[0.16em] text-zinc-500 uppercase"
                                >
                                ...
                                <span
                                    ...
                                    :key="mediaType"
                                    class="inline-flex rounded-full border border-black/8 bg-brand-panel-strong/40 px-2.5 py-1 text-xs font-medium text-zinc-700"
                                >
                                    {{
                                        guestUploadTypeLabels[mediaType] ??
                                        mediaType
                                    }}
                                </span>
                                <Link
                                    :href="`${eventLinks.settings}#guest-upload-types`"
                                    class="text-xs font-medium text-zinc-600 underline-offset-4 hover:text-brand-ink hover:underline"
                                >
                        ...
                        <div
                            class="flex flex-wrap gap-2 lg:max-w-sm lg:justify-end"
                        >
                            <Button
                    ...
                    >
                        ...
                        >
                            <dt
                                class="flex items-center gap-2 text-[0.68rem] font-semibold tracking-[0.18em] text-zinc-500 uppercase"
                            >
                                <component
                                    :is="item.icon"
                            ...
                            <dd
                                class="mt-2 text-sm font-semibold text-brand-ink sm:text-base"
                            >
                                {{ item.value }}
                            </dd>
                ...
                >
                    ...
                    >
                        ...
                        >
                            <div>
                                <h2
                                    class="text-base font-semibold text-brand-ink sm:text-lg"
                                >
                                ...
                                <p class="mt-1 text-sm text-zinc-600">
                                    {{
                                        t(
                                            'event_home.section.recent_description',
                                        )
                                    }}
                                </p>
                            </div>
                        ...
                        <div
                            ...
                            class="py-8 text-sm text-zinc-600"
                        >
                        ...
                        <div
                            v-else
                            class="max-h-[30rem] min-h-0 flex-1 overflow-y-auto pt-2"
                        >
                            <div class="divide-y divide-black/5">
                                ...
                                >
                                    <div class="min-w-0">
                                        <p
                                            class="truncate text-sm text-brand-ink"
                                        >
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="mt-1 text-sm text-zinc-600">
                                            {{
                                                t(
                                                    'event_home.album.description',
                                                )
                                            }}
                                        </p>
                                        <p
                                            class="mt-2 text-xs font-semibold tracking-[0.18em] text-zinc-500 uppercase"
                                        >
                                            {{
                                                t(
                                                    'event_home.album.code_label',
                                                    {
                                                        code: eventLinks.albumAccessCode,
                                                    },
                                                )
                                            }}
                                        </p>
                                        <p class="mt-1 text-xs text-zinc-500">
                                            {{
                                                t(
                                                    'event_home.album.code_hint',
                                                    {
                                                        shortcut:
                                                            eventLinks.albumEntryShortcut,
                                                    },
                                                )
                                            }}
                                        </p>
                                        <p
                                            v-if="eventLinks.albumShortUrl"
                                            class="mt-1 text-xs text-zinc-500"
                                        >
                                            {{
                                                t(
                                                    'event_home.album.short_link',
                                                    {
                                                        url: eventLinks.albumShortUrl,
                                                    },
                                                )
                                            }}
                                        </p>
                                        <div class="mt-3 flex flex-wrap gap-2">
                                            ...
                                            >
                                                <a
                                                    :href="eventLinks.album"
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                >
                                                    {{
                                                        t(
                                                            'event_home.actions.open',
                                                        )
                                                    }}
                                                </a>
                                            </Button>
                                            <Button
                                                ...
                                                @click="
                                                    copyText(
                                                        eventLinks.album,
                                                        t(
                                                            'event_home.clipboard.album_link',
                                                        ),
                                                    )
                                                "
                                            >
                                            ...
                                            <Button
                                                ...
                                                @click="
                                                    copyText(
                                                        eventLinks.albumShortUrl ??
                                                            eventLinks.albumAccessCode,
                                                        eventLinks.albumShortUrl
                                                            ? t(
                                                                  'event_home.clipboard.album_short_link',
                                                              )
                                                            : t(
                                                                  'event_home.clipboard.album_code',
                                                              ),
                                                    )
                                                "
                                            >
                                                ...
                                                {{
                                                    eventLinks.albumShortUrl
                                                        ? t(
                                                              'event_home.actions.short_link',
                                                          )
                                                        : t(
                                                              'event_home.actions.code',
                                                          )
                                                }}
                                            </Button>
                                        </div>
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        >
                                            {{
                                                t(
                                                    'event_home.wall.short_link',
                                                    {
                                                        url: eventLinks.wallShortUrl,
                                                    },
                                                )
                                            }}
                                        </p>
                                        <div class="mt-3 flex flex-wrap gap-2">
                                            ...
                                            <Button
                                                ...
                                                @click="
                                                    copyText(
                                                        eventLinks.wall,
                                                        t(
                                                            'event_home.clipboard.wall_link',
                                                        ),
                                                    )
                                                "
                                            >
                                            ...
                                            <Button
                                                ...
                                                @click="
                                                    copyText(
                                                        eventLinks.wallShortUrl,
                                                        t(
                                                            'event_home.clipboard.wall_short_link',
                                                        ),
                                                    )
                                                "
                                            >
                                            ...
                                            >
                                                <a
                                                    :href="
                                                        eventLinks.wallQrDataUrl
                                                    "
                                                    download="photo-wall-qr.svg"
                                                >
                                                    <Download
                                                        class="mr-2 size-4"
                                                    />
                ...
                >
                    ...
                    >
                        <div class="max-w-3xl">
                            <p
                                class="text-[0.72rem] font-semibold tracking-[0.24em] text-white/60 uppercase"
                            >
                            ...
                            <p
                                class="mt-3 text-sm leading-6 text-white/75 sm:text-base"
                            >
                                {{
                                    t(
                                        'event_home.print_pack.workspace_description',
                                    )
                                }}
                            </p>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-3">
                            <Button
                                as-child
                                class="rounded-full border-0 bg-white text-brand-ink hover:bg-white/90"
                            >
    ...
    <Dialog v-model:open="modalOpen">
        <DialogContent class="sm:max-w-md">
            <DialogHeader>
                <DialogTitle>{{
                    t('event_home.dialog.ready_title')
                }}</DialogTitle>
                <DialogDescription>
        ...
        >
            ...
            >
                ...
                <template v-if="qrPreview === 'album'">
                    <p
                        class="mt-4 text-[0.72rem] font-semibold tracking-[0.18em] text-zinc-500 uppercase"
                    >
                        {{ t('event_home.dialog.album_code_label') }}
                    </p>
                    <p
                        class="mt-2 text-3xl font-semibold tracking-[0.32em] text-brand-ink"
                    >
                        {{ eventLinks.albumAccessCode }}
                    </p>
                    <p class="mt-2 text-sm text-zinc-500">
                        {{
                            t('event_home.dialog.album_code_hint', {
                                shortcut: eventLinks.albumEntryShortcut,
                            })
                        }}
                    </p>
                </template>
