...
it('updates a collaborator role from the settings list', function () {
    $owner = User::factory()->create();
    $event = Event::factory()->for($owner)->create();
    $collaborator = EventCollaborator::query()->create([
        'event_id' => $event->id,
        'email' => 'collab@example.com',
        'role' => 'manager',
        'status' => 'active',
    ...
    $this->actingAs($owner)
        ->from(route('events.settings', $event))
        ->patch(route('events.collaborators.update', [$event, $collaborator]), [
            'role' => 'viewer',
        ])
        ->assertRedirect(route('events.settings', $event))
        ->assertSessionHas('success', 'Collaborator role updated.');

    expect($collaborator->refresh()->role)->toBe('viewer');
});

it('removes a collaborator from the settings list', function () {
    $owner = User::factory()->create();
    $event = Event::factory()->for($owner)->create();
    $collaborator = EventCollaborator::query()->create([
        'event_id' => $event->id,
    ...
    $this->actingAs($owner)
        ->from(route('events.settings', $event))
        ->delete(route('events.collaborators.destroy', [$event, $collaborator]))
        ->assertRedirect(route('events.settings', $event))
        ->assertSessionHas('success', 'Collaborator removed.');
...
it('forbids updating or removing collaborators on another users event', function () {
    $owner = User::factory()->create();
    $otherUser = User::factory()->create();
    $event = Event::factory()->for($owner)->create();
    $collaborator = EventCollaborator::query()->create([
...
it('returns 404 when the collaborator belongs to a different event', function () {
    $owner = User::factory()->create();
    $event = Event::factory()->for($owner)->create();
...
it('lists collaborators with management links on the settings page', function () {
    $owner = User::factory()->create();
    $event = Event::factory()->for($owner)->create();
    $collaborator = EventCollaborator::query()->create([
        'event_id' => $event->id,
    ...
    $this->actingAs($owner)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ->component('events/Settings')
            ->where('eventLinks.collaboratorsStore', route('events.collaborators.store', $event))
            ->where(
                'currentEvent.collaborators',
                fn ($rows) => collect($rows)->contains(fn ($row) => $row['email'] === 'collab@example.com'
                    && $row['links']['update'] === route('events.collaborators.update', [$event, $collaborator])
                    && $row['links']['destroy'] === route('events.collaborators.destroy', [$event, $collaborator])),
            )
        );
});
